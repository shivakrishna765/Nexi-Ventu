...
def add_startup(
    ...
    new_startup = Startup(**startup.model_dump(), founder_id=current_user.id)
    db.add(new_startup)
    db.commit()
...
@router.get("/startup/{startup_id}", response_model=StartupResponse)
...
@router.put("/update-startup/{startup_id}", response_model=StartupResponse)
def update_startup(
    startup_id: str,
    ...
    if not startup:
        raise HTTPException(status_code=404, detail="Startup not found")
    if str(startup.founder_id) != str(current_user.id) and not current_user.is_admin:
